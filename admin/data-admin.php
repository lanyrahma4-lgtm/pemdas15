                ...
                <div class="table-placeholder" style="height: auto; min-height: 300px; padding: 0;">
                    <table>
                        ...
                        <tbody>
                            <?php
                            $admin_data_dummy = [
                                ['id' => 1, 'nama' => 'Farid Thaufiqul Karimah', 'email' => 'farid@polije.ac.id', 'password_hash' => '******'],
                                ['id' => 2, 'nama' => 'Rahma Hazmillah', 'email' => 'rahma@polije.ac.id', 'password_hash' => '******'],
                            ];
                            
                            foreach ($admin_data_dummy as $index => $data) {
                                echo '<tr>';
                                echo '<td>' . ($index + 1) . '</td>';
                                echo '<td>' . htmlspecialchars($data['nama']) . '</td>';
                                echo '<td>' . htmlspecialchars($data['email']) . '</td>';
                                echo '<td>' . htmlspecialchars($data['password_hash']) . '</td>';
                                echo '<td>';
                                echo '<button class="action-btn btn-edit" onclick="alert(\'Edit Admin ID ' . $data['id'] . '\')">edit</button>';
